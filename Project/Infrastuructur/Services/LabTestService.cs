...
namespace HealthcareSystem.Infrastructure.Services
{
    public class LabTestService : ILabTestService
    {
        ...
        public LabTestService(ApplicationDbContext context, ILogger<LabTestService> logger, DatabaseHelpers helper,IEmailService emailService,INotificationService notificationService)
        {
            _context = context;
            _logger = logger;
            _helper = helper;
        ...
        public async Task<LabTestResponse> UpdateLabTestAsync(Guid labTestId, UpdateLabTestRequest request)
        {
            ...
            if (request.Status == LabTestStatus.Completed)
            {
               await  _emailService.SendEmailAsync(new EmailMessage
                ...
                await _notificationService.SendNotificationAsync(labTest.PatientId, NotificationType.LabTestCompleted, "lab test Completed", $"Your {labTest.TestName} results are now available",
        $"/lab-tests/{labTestId}", labTestId.ToString());
            }
