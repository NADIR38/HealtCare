...
namespace HealthcareSystem.Infrastructure.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public DoctorService(
            ...
            ILogger<DoctorService> logger,INotificationService notificationService)
        {
            _context = context;
    ...
    $"Your leave request from {leave.StartDate:MMM dd} to {leave.EndDate:MMM dd} has been approved",
    $"/leaves/{leaveId}",
    leaveId.ToString()
);
            }
            ...
            {
                _logger.LogError(ex, "Failed to send leave approval Notification to ");
            }
     ...
     $"Your leave request from {leave.StartDate:MMM dd} to {leave.EndDate:MMM dd} has been rejected",
     $"/leaves/{leaveId}",
     leaveId.ToString()
 );
            }
