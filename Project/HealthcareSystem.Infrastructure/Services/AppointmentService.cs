...
namespace HealthcareSystem.Infrastructure.Services
{
    ...
    $"Your appointment on {appointment.AppointmentDate:MMMM dd, yyyy} has been cancelled. Reason: {cancellationReason}",
    null,
    ...
    NotificationType.AppointmentCancelled,
    "Appointment Cancelled",
    $"Appointment with {appointment.Patient.User.FirstName} {appointment.Patient.User.LastName} has been cancelled",
    null,
    appointmentId.ToString()
);
            ...
            {
                _logger.LogError(ex, "Failed to send cancellation Notifications");
            }
            return true;
        }
        ...
        {
            var appointment=await  UpdateStatusAsync(appointmentId, AppointmentStatus.CheckedIn, "Patient checked in");
            try {
                await _notificationService.SendNotificationAsync(
    appointment.DoctorId,
    NotificationType.AppointmentCheckIn,
    "Patient Checked In",
    $"{appointment.PatientName} {appointment.PatientName} has checked in for their appointment",
    ...
    appointmentId.ToString()
            ...
            {
                _logger.LogInformation(ex, "Unable to send Notifications");
            }
            return appointment;
        }
        ...
        public async Task<AppointmentResponse> CreateAppointmentAsync(CreateAppointmentRequest request, Guid createdBy)
        {
            ...
            try
            {
                ...
                try
                {
                    // In CreateAppointmentAsync, after creating appointment:
                    await _notificationService.SendNotificationAsync(
                        appointment.PatientId,
                        NotificationType.AppointmentBooked,
                        "Appointment Booked",
                        $"Your appointment with Dr. {doctor.User.FirstName} {doctor.User.LastName} is confirmed for {appointment.AppointmentDate:MMMM dd, yyyy} at {appointment.StartTime:hh\\:mm tt}",
                    ...
                    await _notificationService.SendNotificationAsync(
                        ...
                        $"New appointment with {patient.User.FirstName} {patient.User.LastName} on {appointment.AppointmentDate:MMMM dd, yyyy} at {appointment.StartTime:hh\\:mm tt}",
                        $"/appointments/{appointment.Id}",
                        appointment.Id.ToString()
                    );
